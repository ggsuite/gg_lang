...
- `LanguageCatalog` — the catalog of all language-specific commands
  (install, analyze, format, test, publish, ...) and package metadata, loaded
  from a bundled `languages.json` asset.
...
commands and manifest, then add the matching `ProjectType` value.
